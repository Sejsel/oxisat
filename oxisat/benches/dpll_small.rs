...
use oxisat::dpll::{Clause, Implementation, NoStats, Stats, Variable, CNF};
...
fn dpll_4v_3c(c: &mut Criterion) {
    ...
    group.bench_with_input(
        BenchmarkId::new("cnf_transforming-nostats", "4v-3c"),
        &cnf,
        |b, cnf| b.iter(|| dpll::solve::<NoStats>(cnf, Implementation::CnfTransforming)),
    );

    group.bench_with_input(
        BenchmarkId::new("cnf_transforming-stats", "4v-3c"),
        &cnf,
        |b, cnf| b.iter(|| dpll::solve::<Stats>(cnf, Implementation::CnfTransforming)),
    );
    group.bench_with_input(
        BenchmarkId::new("clause_mapping-nostats", "4v-3c"),
        &cnf,
        |b, cnf| b.iter(|| dpll::solve::<NoStats>(cnf, Implementation::CnfTransforming)),
    );

    group.bench_with_input(
        BenchmarkId::new("clause_mapping-stats", "4v-3c"),
        &cnf,
        |b, cnf| b.iter(|| dpll::solve::<Stats>(cnf, Implementation::CnfTransforming)),
    );
    group.finish();
...
fn dpll_cadical_cnf(c: &mut Criterion) {
    ...
    for instance in ["add4", "prime4", "prime9", "sqrt2809"] {
        let path = format!(
            "{}/tests/cnf/cadical/{instance}.cnf",
            env!("CARGO_MANIFEST_DIR")
        );
        let cnf = load_dimacs(path).into();

        group.bench_with_input(
            BenchmarkId::new("cnf_transforming-nostats", instance),
            &cnf,
            |b, cnf| b.iter(|| dpll::solve::<NoStats>(cnf, Implementation::CnfTransforming)),
        );

        group.bench_with_input(
            BenchmarkId::new("cnf_transforming-stats", instance),
            &cnf,
            |b, cnf| b.iter(|| dpll::solve::<Stats>(cnf, Implementation::CnfTransforming)),
        );

        group.bench_with_input(
            BenchmarkId::new("clause_mapping-nostats", instance),
            &cnf,
            |b, cnf| b.iter(|| dpll::solve::<NoStats>(cnf, Implementation::ClauseMapping)),
        );

        group.bench_with_input(
            BenchmarkId::new("clause_mapping-stats", instance),
            &cnf,
            |b, cnf| b.iter(|| dpll::solve::<Stats>(cnf, Implementation::ClauseMapping)),
        );
    }
    group.finish();
}
